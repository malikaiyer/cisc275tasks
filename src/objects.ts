...
export function isCorrect(question: Question, answer: string): boolean {
    ...
    if (lower === question.expected.toLowerCase()){
        return true;
    }
    return false;
}
...
export function toShortForm(question: Question): string {
    const str2 = question.id + ": " + question.name.slice(0, 10);
    return str2;
}
...
export function toMarkdown(question: Question): string {
    const lineOptions = "- " + question.options.join("\n- ");
    let markStr = "# " + question.name + "\n" + question.body +"\n";
    if (question.type === "multiple_choice_question"){
        markStr += lineOptions;
    }
    else if (question.type === "short_answer_question"){
        markStr = "# " + question.name + "\n" + question.body;
    }
    return markStr;
}
...
export function duplicateQuestion(id: number, oldQuestion: Question): Question {
    const copyQuestion = {...oldQuestion, id: id, name: "Copy of " + oldQuestion.name, published: false};
    return copyQuestion;
}
...
export function mergeQuestion(
    ...
    const mergedQ = {id: id, name: name, body: contentQuestion.body, type: contentQuestion. type, options: contentQuestion.options, expected: contentQuestion.expected, published: false, points: points};
    return mergedQ;
}
